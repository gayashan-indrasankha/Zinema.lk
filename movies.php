...
// Fetch Top 5 Trending Movies for Hero Slider
$hero_movies = [];
$hero_sql = "SELECT * FROM movies ORDER BY views DESC LIMIT 5";
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    if (!empty($movies)) {
        foreach ($movies as $movie) {
            echo '<a href="movie-details.php?id=' . $movie['id'] . '" class="movie-card">';
            echo '<img src="' . htmlspecialchars($movie['cover_image']) . '" alt="' . htmlspecialchars($movie['title']) . '" loading="lazy" decoding="async" class="movie-poster" onerror="this.src=\'assets/images/placeholder.jpg\';">';
            echo '<div class="movie-info">';
            echo '<div class="movie-title">' . htmlspecialchars($movie['title']) . '</div>';
            echo '<div style="font-size: 10px; color: #888;">' . date('Y', strtotime($movie['release_date'])) . '</div>';
            echo '</div>';
            echo '</a>';
        }
    } else {
        // Return empty string if no more movies
    }
    exit;
}
?>
...
<head>
    ...
    <meta name="description" content="Watch the latest movies online in Full HD with Sinhala subtitles and dubbing. Stream action, comedy, drama, horror and more on Zinema.lk - Sri Lanka's #1 movie streaming site.">
    <meta name="keywords" content="sinhala movies, sinhala dubbed movies, watch movies online, sinhala subtitles, HD movies, free movies, Zinema.lk">
    <link rel="canonical" href="<?php echo BASE_URL; ?>/movies.php">
    <link rel="icon" type="image/png" href="<?php echo BASE_URL; ?>/assets/images/tab-logo.png">
    <title>Watch Movies Online - Sinhala Dubbed & Subtitled | Zinema.lk</title>
    
    <!-- Open Graph Tags -->
    <meta property="og:title" content="Watch Movies Online - Sinhala Dubbed & Subtitled | Zinema.lk">
    <meta property="og:description" content="Stream the latest movies in Full HD with Sinhala subtitles and dubbing. Action, comedy, drama, horror and more!">
    <meta property="og:type" content="website">
    <meta property="og:url" content="<?php echo BASE_URL; ?>/movies.php">
    ...
    <style>
        ...
        .simple-movie-grid .movie-card:hover {
        ...
        .simple-movie-grid .movie-card::before {
            content: '▶';
            position: absolute;
            top: 50%;
            left: 50%;
        ...
        .simple-movie-grid .movie-card:hover::before {
            opacity: 1;
            transform: translate(-50%, -50%) scale(1);
        }

        /* Dark Overlay on Hover */
        .simple-movie-grid .movie-card::after {
            content: '';
            position: absolute;
            inset: 0;
        ...
        .simple-movie-grid .movie-card:hover::after {
            opacity: 1;
        }

        .simple-movie-grid .movie-card img {
            width: 100%;
            aspect-ratio: 2/3;
            object-fit: cover;
            display: block;
        ...
        .header-search-form input {
            ...
            color: #fff;
            background: rgba(20, 22, 30, 0.7);
            border: 2px solid rgba(255, 255, 255, 0.08); /* Bottom nav border style */
            border-radius: 50px;
            outline: none;
            transition: all 0.3s ease;
        }
        ...
        .header-search-form input:focus {
            border-color: rgba(42, 108, 255, 0.5); /* Purple/blue border */
            box-shadow: 0 0 15px rgba(42, 108, 255, 0.3), 0 0 30px rgba(255, 51, 102, 0.15); /* Purple/blue glow */
        }
        ...
        /* Add extra bottom padding to movie grid for better spacing */
        .simple-movie-grid {
            padding-bottom: 20px;
        }
    </style>
...
<body>
    ...
    <div class="cs-header">
        ...
        <div class="header-search-bar" id="header-search-bar" style="display: none;">
            <div class="search-wrapper">
                <form action="movies.php" method="GET" class="header-search-form">
    ...
    <div class="app-container">
        <div class="main-content">
            ...
            <div class="language-filter-tabs">
                <a href="movies.php<?php echo $genre_param_first; ?>" class="filter-tab <?php echo !isset($_GET['filter']) ? 'active' : ''; ?>">
                    All
                </a>
                <a href="movies.php?filter=dubbed<?php echo $genre_param; ?>" class="filter-tab <?php echo (isset($_GET['filter']) && $_GET['filter'] == 'dubbed') ? 'active' : ''; ?>">
                    Dubbed
                </a>
                <a href="movies.php?filter=subtitled<?php echo $genre_param; ?>" class="filter-tab <?php echo (isset($_GET['filter']) && $_GET['filter'] == 'subtitled') ? 'active' : ''; ?>">
                    Subtitled
                </a>
            </div>
            ...
            <div class="container">
                <?php if (!empty($movies)): ?>
                    <div class="simple-movie-grid">
                        <?php foreach ($movies as $movie): 
                            $slug = createSlug($movie['title']);
                            $clean_link = "movie/" . $movie['id'] . "/" . $slug;
                            ...
                            if (!preg_match('/^http/', $m_poster)) {
                                $local_check = __DIR__ . '/' . ltrim($m_poster, '/');
                        ...
                        ?>
                            <a href="<?php echo $clean_link; ?>" class="movie-card">
                                ...
                                <div class="movie-info">
                                    ...
                                    <div style="font-size: 10px; color: #888;"><?php echo date('Y', strtotime($movie['release_date'])); ?></div>
                                </div>
                            </a>
                        <?php endforeach; ?>
                    </div>
                <?php else: ?>
                    <div class="no-movies">
                        ...
                        <p>We couldn't find any movies matching your search. Try a different keyword!</p>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <div class="drawer-content">
            ...
            <div class="drawer-section">
                ...
                <div class="genre-list">
                    ...
                    <a href="movies.php?genre=Action" class="genre-item <?php echo ($current_genre === 'Action') ? 'active' : ''; ?>">Action</a>
                    <a href="movies.php?genre=Comedy" class="genre-item <?php echo ($current_genre === 'Comedy') ? 'active' : ''; ?>">Comedy</a>
                    <a href="movies.php?genre=Drama" class="genre-item <?php echo ($current_genre === 'Drama') ? 'active' : ''; ?>">Drama</a>
                    <a href="movies.php?genre=Horror" class="genre-item <?php echo ($current_genre === 'Horror') ? 'active' : ''; ?>">Horror</a>
                    <a href="movies.php?genre=Romance" class="genre-item <?php echo ($current_genre === 'Romance') ? 'active' : ''; ?>">Romance</a>
                    <a href="movies.php?genre=Sci-Fi" class="genre-item <?php echo ($current_genre === 'Sci-Fi') ? 'active' : ''; ?>">Sci-Fi</a>
                    <a href="movies.php?genre=Thriller" class="genre-item <?php echo ($current_genre === 'Thriller') ? 'active' : ''; ?>">Thriller</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Only active on mobile (< 768px for safety, or check if pagination is hidden)
            // The request was "Mobile users must be able to see ALL available movies by scrolling"
            
            const movieGrid = document.querySelector('.simple-movie-grid');
            if (!movieGrid) return;
            ...
            // Check if we are on mobile (CSS media query threshold is 768px)
            function isMobile() {
            ...
            // Create a sentinel element to track scrolling
            ...
            // Append sentinel after the grid container
            ...
            function loadMoreMovies() {
                if (isLoading || currentPage >= totalPages || !isMobile()) return;
                
                isLoading = true;
                
                // Show loader (optional, or just reuse sentinel as loader holder)
                sentinel.innerHTML = '<div style="text-align:center; padding: 10px; color: #888;"><i class="fas fa-spinner fa-spin"></i> Loading more movies...</div>';
                ...
                fetch('movies.php?' + params.toString())
                    ...
                    .then(html => {
                        if (html.trim().length > 0) {
                            ...
                            // No more content
                            scrollObserver.unobserve(sentinel);
                            sentinel.innerHTML = '<div style="text-align:center; padding: 20px; color: #888;">You have reached the end.</div>';
                        }
                        isLoading = false;
                    ...
                    .catch(error => {
                        ...
                        isLoading = false;
                        sentinel.innerHTML = '<div style="text-align:center; color: red;">Error loading movies. Tap to retry.</div>';
                        sentinel.onclick = () => { sentinel.onclick = null; loadMoreMovies(); };
                    });
            }
        });
                    ...
                    .then(movies => {
                        if (movies.length === 0) {
                            ...
                            resultsContainer.innerHTML = movies.map(movie => `
                                <a href="movie-details.php?id=${movie.id}" class="search-result-item">
        ...
        function openSearchOverlay() {
            ...
            overlay.classList.add('active'); // Use class for animation
    ...
    <div id="search-overlay" class="search-overlay">
        <div class="search-box-wrapper">
            ...
            <form action="movies.php" method="GET" class="search-form-overlay">
                <input type="text" name="search" id="search-input" placeholder="Search movies..." autocomplete="off">
            </form>
        </div>
    </div>
